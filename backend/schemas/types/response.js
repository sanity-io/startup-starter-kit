        ...
        annotations: [
          {
            ...
            blockEditor: {
              icon: () => '🔊',
              render: ({children, volume}) => <span>{children} {['x-loud','loud'].includes(volume) ? '🔊' : '🔈'}</span>
            },
            fields: [
              {
                name: 'rate',
          ...
          },
          {
            title: 'Alias (sub)',
            name: 'sub',
            description: 'The sub element is employed to indicate that the text in the alias attribute value replaces the contained text for pronunciation. This allows a document to contain both a spoken and written form.',
            type: 'object',
            blockEditor: {
              icon: () => '🔤',
